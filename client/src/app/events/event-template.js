...
const getRefreshedImageUrl = (imageUrl) => {
  return imageUrl; // Remove cache busting to improve caching
};
...
const EventGallery = React.memo(({ images, title }) => {
  return (
    <div className="mt-8">
  ...
  function EventPage() {
    ...
    useEffect(() => {
      ...
      async function loadImages() {
        try {
          setLoading(true);
          const response = await fetch(`/api/getEventImages?folder=${eventData.folderName}`);
